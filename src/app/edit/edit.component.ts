...
export class EditComponent implements OnInit {
  ...
  student={"_id":"","name":"","mobileNo":"","dob":"","age":0,"ad1":"","ad2":"","state":"","city":"","pincode":0,"fname":"","mname":"","check":false,"textboxarea":"","createdAt":"2020-09-12T08:38:25.528Z","updatedAt":"2020-09-12T08:38:25.528Z","__v":0}
  ...
  ngOnInit(): void {
    ...
    this.userService.getstudentdata(this.router.url.split('/')[2]).subscribe(suc=>{this.student=suc.student;this.state=suc.student.state},err=>{})
  ...
  submitSignUp() {
    this.signUpForm.value.check=this.student.check
    this.signUpForm.value.age=this.agecal()
    if(this.signUpForm.value.check==="") this.signUpForm.value.check=false
    if(this.signUpForm.value.check==false) this.signUpForm.value.textboxarea=""
    
    this.signUpForm.value._id=this.student._id
    if(this.signUpForm.value.name=="") this.signUpForm.value.name=this.student.name
    if(this.signUpForm.value.dob=="") this.signUpForm.value.dob=this.student.dob
    if(this.signUpForm.value.mobileNo=="") this.signUpForm.value.mobileNo=this.student.mobileNo
    if(this.signUpForm.value.ad1=="") this.signUpForm.value.ad1=this.student.ad1
    if(this.signUpForm.value.ad2=="") this.signUpForm.value.ad2=this.student.ad2
    if(this.signUpForm.value.state=="") this.signUpForm.value.state=this.student.state
    if(this.signUpForm.value.city=="") this.signUpForm.value.city=this.student.city
    if(this.signUpForm.value.pincode=="") this.signUpForm.value.pincode=this.student.pincode
    if(this.signUpForm.value.fname=="") this.signUpForm.value.fname=this.student.fname
    if(this.signUpForm.value.mname=="") this.signUpForm.value.mname=this.student.mname
    if(this.signUpForm.value.textboxarea=="") this.signUpForm.value.textboxarea=this.student.textboxarea
    if(this.signUpForm.value.check==false) this.signUpForm.value.textboxarea=""

     this.userService.putStudent(this.signUpForm.value).subscribe(
         suc=>{ if(suc.status==true) this.router.navigate(['/userprofile']);},err=>{}
  ...
  agecal(){
    ...
    var dateofbirth=new Date(this.signUpForm.value.dob)
   
    if(!Math.floor((new Date().getTime()-dateofbirth.getTime())/(24 * 60 * 60 * 1000*365)))
    dateofbirth=new Date(this.student.dob)
    return Math.floor((new Date().getTime()-dateofbirth.getTime())/(24 * 60 * 60 * 1000*365))
   }
